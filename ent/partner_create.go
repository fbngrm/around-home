...
func (pc *PartnerCreate) SetRating(i int) *PartnerCreate {
	pc.mutation.SetRating(i)
	return pc
}

// SetRadiusOfOperation sets the "radiusOfOperation" field.
func (pc *PartnerCreate) SetRadiusOfOperation(f float64) *PartnerCreate {
	pc.mutation.SetRadiusOfOperation(f)
	return pc
}

// AddMaterialIDs adds the "materials" edge to the Material entity by IDs.
func (pc *PartnerCreate) AddMaterialIDs(ids ...int) *PartnerCreate {
	pc.mutation.AddMaterialIDs(ids...)
	return pc
}

// AddMaterials adds the "materials" edges to the Material entity.
func (pc *PartnerCreate) AddMaterials(m ...*Material) *PartnerCreate {
	ids := make([]int, len(m))
	for i := range m {
		ids[i] = m[i].ID
	}
	return pc.AddMaterialIDs(ids...)
}
...
func (pc *PartnerCreate) check() error {
	...
	if _, ok := pc.mutation.Rating(); !ok {
		return &ValidationError{Name: "rating", err: errors.New(`ent: missing required field "Partner.rating"`)}
	}
	if _, ok := pc.mutation.RadiusOfOperation(); !ok {
		return &ValidationError{Name: "radiusOfOperation", err: errors.New(`ent: missing required field "Partner.radiusOfOperation"`)}
	}
	return nil
}
...
func (pc *PartnerCreate) createSpec() (*Partner, *sqlgraph.CreateSpec) {
	...
	if value, ok := pc.mutation.RadiusOfOperation(); ok {
		_spec.Fields = append(_spec.Fields, &sqlgraph.FieldSpec{
			Type:   field.TypeFloat64,
			Value:  value,
			Column: partner.FieldRadiusOfOperation,
		})
		_node.RadiusOfOperation = value
	}
	if nodes := pc.mutation.MaterialsIDs(); len(nodes) > 0 {
