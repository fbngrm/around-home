...
func (mq *MaterialQuery) QueryPartners() *PartnerQuery {
...
func (mq *MaterialQuery) Clone() *MaterialQuery {
	...
	return &MaterialQuery{
		...
		withPartners: mq.withPartners.Clone(),
...
func (mq *MaterialQuery) WithPartners(opts ...func(*PartnerQuery)) *MaterialQuery {
	query := &PartnerQuery{config: mq.config}
	for _, opt := range opts {
		opt(query)
	}
	mq.withPartners = query
	return mq
}
...
//		GroupBy(material.FieldMaterial).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (mq *MaterialQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Material, error) {
	...
	if query := mq.withPartners; query != nil {
		if err := mq.loadPartners(ctx, query, nodes,
			func(n *Material) { n.Edges.Partners = []*Partner{} },
			func(n *Material, e *Partner) { n.Edges.Partners = append(n.Edges.Partners, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (mq *MaterialQuery) loadPartners(ctx context.Context, query *PartnerQuery, nodes []*Material, init func(*Material), assign func(*Material, *Partner)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Material)
	nids := make(map[int]map[*Material]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(material.PartnersTable)
		s.Join(joinT).On(s.C(partner.FieldID), joinT.C(material.PartnersPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(material.PartnersPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(material.PartnersPrimaryKey[1]))
	...
	neighbors, err := query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
		...
		spec.Assign = func(columns []string, values []interface{}) error {
			...
			if nids[inValue] == nil {
				nids[inValue] = map[*Material]struct{}{byID[outValue]: struct{}{}}
	...
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "partners" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
