...
func (pq *PartnerQuery) QueryMaterials() *MaterialQuery {
	query := &MaterialQuery{config: pq.config}
...
func (pq *PartnerQuery) Clone() *PartnerQuery {
	...
	return &PartnerQuery{
		...
		withMaterials: pq.withMaterials.Clone(),
...
func (pq *PartnerQuery) WithMaterials(opts ...func(*MaterialQuery)) *PartnerQuery {
	query := &MaterialQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withMaterials = query
	return pq
}
...
func (pq *PartnerQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Partner, error) {
	...
	if query := pq.withMaterials; query != nil {
		if err := pq.loadMaterials(ctx, query, nodes,
			func(n *Partner) { n.Edges.Materials = []*Material{} },
			func(n *Partner, e *Material) { n.Edges.Materials = append(n.Edges.Materials, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (pq *PartnerQuery) loadMaterials(ctx context.Context, query *MaterialQuery, nodes []*Partner, init func(*Partner), assign func(*Partner, *Material)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[int]*Partner)
	nids := make(map[int]map[*Partner]struct{})
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(partner.MaterialsTable)
		s.Join(joinT).On(s.C(material.FieldID), joinT.C(partner.MaterialsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(partner.MaterialsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(partner.MaterialsPrimaryKey[0]))
	...
	neighbors, err := query.sqlAll(ctx, func(_ context.Context, spec *sqlgraph.QuerySpec) {
		...
		spec.Assign = func(columns []string, values []interface{}) error {
			...
			if nids[inValue] == nil {
				nids[inValue] = map[*Partner]struct{}{byID[outValue]: struct{}{}}
